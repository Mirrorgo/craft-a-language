...
export class TypeUtil{
     ...
     * @param values 
     */
    static createArrayTypeByValues(values:[]):ArrayType{
        let types : ValueType[] = [];
        for(let v of values){
    ...
    //返回typeof运算符的计算结果
    static evaluateTypeOf(t:Type):string{
        let typeStr:string;
    ...
    //比较两个Type对象是否相等。
    private static isSame(t1:Type, t2:Type):boolean{
        if(t1 === t2){
            return true;
        }
     ...
     * @param t 
     */
    private static contains(types:Type[], t:Type):boolean{
        for (let t1 of types){
...
export abstract class TypeVisitor{
    ...
    abstract visitArrayType(t:ArrayType):any;
