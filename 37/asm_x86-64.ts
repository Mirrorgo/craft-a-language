...
import {FunctionSymbol, VarSymbol, built_ins, FunctionKind, ClassSymbol} from './symbol'
import {AstVisitor, AstNode, Block, Prog, VariableDecl, FunctionDecl, FunctionCall, Statement, Expression, ExpressionStatement, Binary, IntegerLiteral, DecimalLiteral, StringLiteral, Variable, ReturnStatement, IfStatement, Unary, ForStatement, ArrayLiteral, IndexedExp, DotExp, ThisExp} from './ast';
...
class FunctionOprand extends Oprand{
    ...
    isMethodOrConstructor:boolean;
    functionName:string|null;
    constructor(functionType:FunctionType, args:Oprand[], functionName:string|null = null, functionAddress:MemAddress|null = null, isConstructorOrMethod:boolean=false){
...
class AsmModule{
    ...
    //函数中每个变量被分配的内存地址（都是相对于rbp的偏移量）
    fun2StackLayout:Map<FunctionSymbol, Map<number,MemAddress>> = new Map();

    //函数中每个变量的类型  todo 这里没有使用窄化后的类型
    fun2VarTyps:Map<FunctionSymbol, Map<number, Type>> = new Map();
    ...
    toString():string{
        ...
        for (let fun of this.fun2Code.keys()){
            //浮点数字面量是以函数为单位的
            str += this.writeDoubleLiterals(fun, funIndex);
        ...
        for (let fun of this.fun2Code.keys()){
            str += this.writeFunctionMeta(fun);
        }

        //字符串字面量是以模块为单位的
        str += this.writeStringLiterals();

        return str;
    }

    writeDoubleLiterals(fun:FunctionSymbol, funIndex:number):string{
    ...
    //34节
    //把函数的栈桢布局信息输出到一个数据段
    writeFunctionMeta(fun:FunctionSymbol):string{
        let stackLayout = this.fun2StackLayout.get(fun) as Map<number, MemAddress>;

        let str = "";
        let varTypes = this.fun2VarTyps.get(fun) as Map<number, Type>;
        let sectionName = "_"+fun.fullName+".meta";
        str += "\t.section	__DATA,__const\n";
        str += "\t.globl\t" + sectionName + "\t\t\t\t## can be accessed globally\n";   //可以被外部函数所引用
        str += "\t.p2align	3\t\t\t\t\t## 8 byte alignment\n";
        str += sectionName + ":\n";

        //先记录函数名称
        str += "\t.quad\t"+"_"+fun.fullName+".name\t\t\t\t## link to function name section\n";

        let maxIndex = 0;
        let varCount = stackLayout.size;
        //获取最大的key值
        for (let i of stackLayout.keys()){
            if (i > maxIndex) maxIndex = i;
        }
        
        //写入变量的数量。如果有更多的信息，也可以写在这里
        str += "\t.quad\t"+varCount +"\t\t\t\t\t\t## var count\n";

        //写入变量信息
        for (let i = 0; i<= maxIndex; i++){   
            if (stackLayout.has(i)){
                if (varTypes.has(i)){
                    let memAddress = stackLayout.get(i) as MemAddress;
                    let typeTag:number  = this.getTypeFlag(varTypes.get(i) as Type);
                    let addressOffset = -memAddress.offset;
                    let hexStr = this.compactToHexString(i,typeTag, addressOffset);
                    str += "\t.quad\t" + hexStr+"\t\t## var" + i + ", type: "+ typeTag + ", address offset: " + addressOffset +"\n"; 
                }
                else{
                    console.log("Cannot find type for var" +i);
                }
            }            
        }
        ...
        str += this.writeFunctionName(fun);
        return str;
    }

    private writeFunctionName(fun:FunctionSymbol):string{
        let str = "";

        //写下函数名称
        let sectionName = "_"+fun.fullName+".name";
        str += "\t.section	__TEXT,__const\n";
        str += "\t.globl\t" + sectionName + "\t\t\t\t## can be accessed globally\n";   //可以被外部函数所引用
        str += "\t.p2align	3\t\t\t\t\t## 8 byte alignment\n";
        str += sectionName + ":\n";
        str += "\t.asciz\t\""+fun.fullName+"\"\n";
        str += "\n";
    ...
    //typeTag: 占8位 1个字节
    //offset：占24位 3个字节
    private compactToHexString(varIndex:number, typeTag:number, offset:number):string{
        let str1 = varIndex.toString(16).padStart(8,"0");
        let str2 = typeTag.toString(16).padStart(2,"0");
        let str3 = offset.toString(16).padStart(6,"0");
        return "0x"+str1+str2+str3;
    }
...
class TempStates{
    ...
    //每个变量的类型，包括临时变量
    //todo：当前算法使用窄化后的类型，以后应该更新
    varTypes:Map<number, Type> = new Map();
...
class AsmGenerator extends AstVisitor{
    ...
    //This表达式总是返回变量0
    private ThisOprand = new VarOprand(0, "this", CpuDataType.int64);
    ...
    private handleFunction(functionSym:FunctionSymbol, block:Block):any{
        ...
        //保存第1个变量的类型
        if (functionSym.classSym) this.s.varTypes.set(0,functionSym.classSym.theType);
        ...
        this.asmModule?.fun2VarTyps.set(this.s.functionSym, this.s.varTypes);
    ...
    visitBinary(bi:Binary):any{
        ...
        assert(typeof left == 'object', "binary左边表达式没有返回Oprand。");
        assert(typeof right == 'object', "binary右边表达式没有返回Oprand。");
        ...
        if (bi.op == Op.Plus || bi.op == Op.Minus || bi.op == Op.Multiply || bi.op == Op.Divide){
            if (!(dest instanceof VarOprand)){
                ...
                this.s.varTypes.set((dest as VarOprand).varIndex, bi.exp1.theType as Type);
        ...
        switch(bi.op){
            ...
            case Op.Assign: //'='
                ...
                if (dest instanceof VarOprand){
                    ...
                    this.s.varTypes.set(tempVar.varIndex, bi.exp2.theType as Type);
    ...
    visitUnary(u:Unary):any{
        ...
        if(u.op == Op.Inc || u.op == Op.Dec){
            ...
            if(u.isPrefix){  //前缀运算符
                ...
                this.s.varTypes.set(tempVar.varIndex, u.theType as Type);
    ...
    visitVariable(variable:Variable):any{
        if (this.s.functionSym !=null && variable.sym!=null){
            if (variable.sym instanceof VarSymbol){
                ...
                this.s.varTypes.set(varIndex, variable.theType as Type);
             ...
             */
            else if (variable.sym instanceof FunctionSymbol){
    ...
    visitStringLiteral(stringLiteral:StringLiteral):any{
        ...
        if (this.asmModule != null){
            ...
            this.s.varTypes.set(tempVar.varIndex, stringLiteral.theType as Type);
        ...
        if (arrOprand.kind == OprandKind.varIndex){
            ...
            for (let i =0; i< literal.exps.length; i++){
                ...
                if (src.kind == OprandKind.doubleIndex){
                    ...
                    this.s.varTypes.set(tempVar.varIndex, SysTypes.Number);
    ...
    visitIndexedExp(exp:IndexedExp):any{
        ...
        if (obj instanceof VarOprand){
            ...
            let tempVar = this.allocateTempVar(CpuDataType.double);
            this.s.varTypes.set(tempVar.varIndex, obj.baseExp.theType as Type);
    ...
    visitFunctionCall(functionCall:FunctionCall, obj:any):any{
        ...
        if (functionCall.sym instanceof FunctionSymbol){
            functionSym = functionCall.sym as FunctionSymbol;
            //内置函数
        ...
        //函数变量
        else if (functionCall.sym instanceof VarSymbol && this.s.functionSym){
            ...
            let functionAddressVar = new VarOprand(varIndex, varSym.name, getCpuDataType(varSym.theType as Type));
            let functionAddress = new MemAddress(functionAddressVar,0);
            let insts = this.getCurrentBB().insts;
            
            assert(varSym.theType instanceof FunctionType, "函数变量的类型应该是FunctionType");
            let functionType = varSym.theType as FunctionType;
    ...
    private processFunctionCall(functionSym:FunctionSymbol, args:Oprand[], isTailCall:boolean, isTailRecursive:boolean, obj:any):any{
        ...
        insts.push(new Inst_1(op, new FunctionOprand(functionSym.theType as FunctionType, args, functionSym.name, functionAddress, functionSym.isMethodOrConstructor)));

        //把返回值拷贝到一个临时变量，并返回这个临时变量
        //并且要重新装载溢出的变量        
        let rtn:Oprand|undefined;
        if (!isTailCall && !isTailRecursive){
            rtn = this.getFuncitonCallResult(functionSym.theType as FunctionType);
        }
    ...
    private getFuncitonCallResult(functionType:FunctionType):VarOprand|undefined{
        let insts = this.getCurrentBB().insts;

        //把结果放到一个新的临时变量里
        let dest:VarOprand|undefined = undefined; 
        if(functionType.returnType != SysTypes.Void){ //函数有返回值时
            ...
            this.s.varTypes.set(dest.varIndex, functionType.returnType as Type);
    ...
    visitDotExp(dotExp:DotExp):any{
        ...
        assert(object, "DotExp左边应该返回一个对象引用");

        let rtn:Oprand;

        if (dotExp.property instanceof Variable){ //属性
            //计算属性在内存中的偏移量
            let varSym = dotExp.property.sym as VarSymbol;
            let classSym = varSym.classSym as ClassSymbol;
            let propIndex = classSym.getPropIndex(varSym);
            assert(propIndex != -1, "在类中找不到属性"+dotExp.property.name+"。");
 ...
 */
 ...
 class Lower{
    ...
    private lowerFunction(functionSym:FunctionSymbol, bbs:BasicBlock[], funIndex:number):BasicBlock[]{
        ...
        //保存变量在栈桢中的布局信息
        this.asmModule?.fun2StackLayout.set(functionSym, this.spilledVars2Address);
    ...
    private lowerParams(){
        ...
        //如果是方法，参数数量加1
        let isMethod:boolean = (this.functionSym as FunctionSymbol).classSym != null;
    ...
    private initStates(functionSym:FunctionSymbol){
        ...
        this.spillOffset = 8;   //前8个字节保存到函数元数据的地址
        this.spilledVars2Address = new Map();  //旧值赋给AsmModule了。
    ...
    private addPrologue(insts:Inst[]):Inst[]{
        ...
        //34节
        //添加函数的元数据地址，在紧跟着rbp的8个字节
        let functionMetaLabel = "_"+this.functionSym?.fullName+".meta@GOTPCREL(%rip)";
        let functionMetaAddress = new Oprand(OprandKind.label, functionMetaLabel);
        newInsts.push(new Inst_2(OpCode.movq, functionMetaAddress, Register.rax)); //在程序一开头，临时用一下rax没问题  
        newInsts.push(new Inst_2(OpCode.movq, Register.rax, new MemAddress(Register.rbp,-8)));
    ...
    private addEpilogue(newInsts:Inst[], inst:Inst = new Inst_0(OpCode.retq)){
        ...
        //添加一些调试信息
        // if (this.functionSym?.name == "main"){
        //     newInsts.push(new Inst_2(OpCode.movq, new Oprand(OprandKind.label, "_bar.meta@GOTPCREL(%rip)"), Register.rdi));
        //     newInsts.push(new Inst_1(OpCode.callq, new Oprand(OprandKind.label, "_println_l")));
            
        //     newInsts.push(new Inst_2(OpCode.movq, new Oprand(OprandKind.label, "_bar.name@GOTPCREL(%rip)"), Register.rdi));
        //     newInsts.push(new Inst_1(OpCode.callq, new Oprand(OprandKind.label, "_println_l")));
            
        //     newInsts.push(new Inst_2(OpCode.movq, new Oprand(OprandKind.label, "_foo.meta@GOTPCREL(%rip)"), Register.rdi));
        //     newInsts.push(new Inst_1(OpCode.callq, new Oprand(OprandKind.label, "_println_l")));
            
        //     newInsts.push(new Inst_2(OpCode.movq, new Oprand(OprandKind.label, "_foo.name@GOTPCREL(%rip)"), Register.rdi));
        //     newInsts.push(new Inst_1(OpCode.callq, new Oprand(OprandKind.label, "_println_l")));
        // }
    ...
    private lowerFunctionCall(inst_1:Inst_1, liveVars:number[], liveVarsAfterCall:number[], newInsts:Inst[]):number[]{
        ...
        for (let j = 0; j < numArgs; j++){
            let paramIndex = j;
            if (functionOprand.isMethodOrConstructor) paramIndex-=1;
...
function getCpuDataType(t:Type):CpuDataType{
    ...
    else if (t instanceof NamedType){  //自定义对象
        dataType = CpuDataType.int64;  
    }
    else if (t instanceof FunctionType){
        dataType = CpuDataType.int64;
