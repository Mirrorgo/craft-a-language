...
import {Symbol, FunctionSymbol, VarSymbol, SymKind, FUN_println, SymbolVisitor, SymbolDumper, built_ins} from './symbol'
...
import { TypeVisitor,NamedType, UnionType,FunctionType,Type, SysTypes, TypeKind, SimpleType} from './types';
...
export class BCGenerator extends AstVisitor{
    ...
    visitIntegerLiteral(integerLiteral: IntegerLiteral):any{
        ...
        let ret:number[] = [];
        let value = integerLiteral.value as number;
        //0-5之间的数字，直接用快捷指令
        if (value >= 0 && value <= 5) {
            switch (value) {
                case 0:
...
export class BCModuleWriter{
    ...
    write(bcModule:BCModule):number[]{
        ...
        for(let c of bcModule.consts){
            if (typeof c == 'number'){
                ...
                if (!built_ins.has(functionSym.name)){ //不用写入系统函数
        ...
        for (let t of this.types){
            ...
            else if (t.kind == TypeKind.Named){
                bc1 = bc1.concat(this.writeSimpleType(t as NamedType));
            }
    ...
    private writeVarSymbol(sym:VarSymbol):number[]{
        ...
        //写入类型名称
        this.writeString(bc, sym.theType.toString());
    ...
    writeFunctionSymbol(sym:FunctionSymbol):number[]{
        ...
        this.writeString(bc, this.types.indexOf(sym.theType).toString());
    ...
    writeSimpleType(t:NamedType):number[]{
        let bc:number[] = [];
    ...
    writeFunctionType(t:FunctionType):number[]{
        ...
        this.writeString(bc, this.types.indexOf(t).toString());

        //写入返回值编号
        if (this.types.indexOf(t.returnType)  == -1){
            this.types.push(t.returnType);
        }
        this.writeString(bc, this.types.indexOf(t.returnType).toString());
        ...
        for (let pt of t.paramTypes){
            ...
            this.writeString(bc, this.types.indexOf(pt).toString());
        }
    ...
    writeUnionType(t:UnionType):number[]{
        ...
        for (let ut of t.types){
            ...
            this.writeString(bc, this.types.indexOf(ut).toString());
        }
        ...
        for(let typeName of this.typeInfos.keys()){
            ...
            if (t.kind == TypeKind.Named){
                ...
                for (let utName of types){
                    let ut = this.types.get(utName) as SimpleType;
