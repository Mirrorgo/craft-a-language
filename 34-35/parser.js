...
class Parser {
    ...
    parseStatement() {
        ...
        //根据'return'关键字，解析return语句
            ...
            t.code == scanner_1.Keyword.This) {
            return this.parseExpressionStatement();
        }
        else if (t.code == scanner_1.Keyword.Super) {
        ...
        else if (t.code == scanner_1.Keyword.Class) {
    ...
    parseType() {
        ...
        if (t1.code == scanner_1.Seperator.OpenParen) {
            ...
            else if (t2.code == scanner_1.Seperator.CloseParen) { //参数列表为空的情况，比如 ()=>number
                rtn = this.parseFunctionType();
            }
            else {
                rtn = this.parseUnionOrIntersectionOrPrimaryType();
    ...
    parsePrimTypeExp() {
        ...
        //预定义类型
        else if (t.code == scanner_1.Keyword.Any || t.code == scanner_1.Keyword.Boolean || t.code == scanner_1.Keyword.String || t.code == scanner_1.Keyword.Number || t.code == scanner_1.Keyword.Void) {
            this.scanner.next();
    ...
    parseFunctionDecl() {
        ...
        let functionKind = symbol_1.FunctionKind.Function;
        let firstToken = this.scanner.peek();
        if (firstToken.code == scanner_1.Keyword.Function) {
            this.scanner.next(); //跳过'function'
        }
        else if (firstToken.code == scanner_1.Keyword.Constructor) {
            functionKind = symbol_1.FunctionKind.Constructor;
        }
        else {
            functionKind = symbol_1.FunctionKind.Method;
        }
        let tName = this.scanner.next();
        if (tName.kind != scanner_1.TokenKind.Identifier && tName.code != scanner_1.Keyword.Constructor) {
        ...
        return new ast_1.FunctionDecl(beginPos, tName.text, callSignature, functionBody, functionKind, isErrorNode);
    }
    /**
     * 解析函数签名
     * callSignature: '(' parameterList? ')' typeAnnotation? ;
   ...
   */
    ...
    parsePrimary() {
        ...
        else if (t.code == scanner_1.Keyword.This) {
            this.scanner.next();
            exp = new ast_1.ThisExp(t.pos);
        }
        else if (t.code == scanner_1.Keyword.Super) {
            exp = new ast_1.SuperExp(t);
        }
        ...
        else if (t.code == scanner_1.Keyword.New) {
            this.scanner.next();
            let t1 = this.scanner.peek();
            let t2 = this.scanner.peek2();
            if (t1.kind == scanner_1.TokenKind.Identifier && t2.code == scanner_1.Seperator.OpenParen) {
                exp = this.parseFunctionCall();
            }
            else {
                this.addError("Invalid syntax to 'New' a object", t1.pos);
                exp = new ast_1.ErrorExp(t.pos, t1.pos);
                this.skip();
            }
        }
        ...
        t = this.scanner.peek();
        //解析Array表达式和点符号表达式
        while (t.code == scanner_1.Seperator.OpenBracket || t.code == scanner_1.Op.Dot) {
            if (t.code == scanner_1.Seperator.OpenBracket) {
                this.scanner.next(); //跳过'['
                let exp1 = this.parseExpression();
                let isErrorNode = (this.scanner.peek().code != scanner_1.Seperator.CloseBracket);
                if (isErrorNode) {
                ...
                this.scanner.next(); //跳过'.'
                let exp1 = this.parsePrimary();
                exp = new ast_1.DotExp(beginPos, this.scanner.getLastPos(), exp, exp1);
            }
            t = this.scanner.peek();
        }
        return exp;
    ...
    parseClassDecl() {
        ...
        if (tName.kind == scanner_1.TokenKind.Identifier) {
            this.scanner.next();
        }
        else {
        ...
        if (tExtends.code == scanner_1.Keyword.Extends) {
            this.scanner.next(); //跳过'extends'
            let tSuper = this.scanner.peek();
            if (tSuper.kind == scanner_1.TokenKind.Identifier) {
        ...
        if (t1.code == scanner_1.Seperator.OpenBrace) { //'{'
            ...
            this.skip();
            body = new ast_1.Block(t1.pos, t1.pos, [], true);
        }
        return new ast_1.ClassDecl(classToken, this.scanner.getLastPos(), tName.text, body, superClass, isErrorNode);
    }
    parseClassBody() {
        let isErrorNode = false;
        let beginPos = this.scanner.getNextPos();
        ...
        let stmts = [];
        let t1 = this.scanner.peek();
        while (t1.code == scanner_1.Keyword.Constructor || t1.kind == scanner_1.TokenKind.Identifier) {
            if (t1.code == scanner_1.Keyword.Constructor) {
                stmts.push(this.parseFunctionDecl());
            }
            else if (t1.kind == scanner_1.TokenKind.Identifier) {
                let t2 = this.scanner.peek2();
                if (t2.code == scanner_1.Seperator.OpenParen) { //'(' 函数声明
                    stmts.push(this.parseFunctionDecl());
                }
                else {
                    ...
                    if (t3.code == scanner_1.Op.Assign) { //'='
                        this.scanner.next();
                        init = this.parseExpression();
                    }
                    let varDecl = new ast_1.VariableDecl(t1.pos, this.scanner.getLastPos(), t1.text, typeExp, init);
                    t3 = this.scanner.peek();
                    let errorVarDecl = false;
                    if (t3.code == scanner_1.Seperator.SemiColon) {
                        this.scanner.next();
                    }
                    else {
                        this.skip();
                    ...
                    stmts.push(new ast_1.VariableStatement(t1.pos, this.scanner.getLastPos(), varDecl, errorVarDecl));
                }
            }
            else {
        ...
        t1 = this.scanner.peek();
        if (t1.code == scanner_1.Seperator.CloseBrace) { //'}'
            this.scanner.next();
        }
        else {
        ...
        return new ast_1.Block(beginPos, this.scanner.getLastPos(), stmts, isErrorNode);
    }
