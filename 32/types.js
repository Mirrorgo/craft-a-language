...
exports.TypeVisitor = exports.SysTypes = exports.IntersectionType = exports.UnionType = exports.FunctionType = exports.ArrayType = exports.ValueType = exports.ComplementNamedType = exports.NamedType = exports.Type = exports.TypeUtil = exports.TypeKind = void 0;
var TypeKind;
(function (TypeKind) {
...
class TypeUtil {
    ...
    */
    ...
    /**
     * 根据数字字面量来推断类型
     * todo这里只是创建值类型，是否需要合并成NamedType？
     * @param values
     */
    static createArrayTypeByValues(values) {
        let types = [];
        for (let v of values) {
            let t = TypeUtil.createTypeByValue(v);
            if (!TypeUtil.contains(types, t))
                types.push(t);
        }
    ...
    static evaluateTypeOf(t) {
        ...
        if (t instanceof NamedType) {
            ...
            else if (t === SysTypes.UndefinedType) {
                typeStr = 'undefined';
            }
            else if (t === SysTypes.Any) { //对于Any类型，如果类型还没有窄化，那么返回undefined.
                typeStr = 'undefined';
            }
            else { //todo 未来还要扩展到支持Symbol
                typeStr = 'object';
            }
        }
        else if (t instanceof ValueType) {
            if (!t.isComplement) {
                typeStr = TypeUtil.evaluateTypeOf(t.typeOfValue);
            }
            else {
                typeStr = 'undefined'; //补集只是表示了一个范围，如果没有窄化，则返回undefined.
            }
        }
            ...
            let t1 = TypeUtil.getBestCommonType(t); //看看是不是可以得到一个NamedType
            if (t1 instanceof NamedType) {
                typeStr = TypeUtil.evaluateTypeOf(t1);
            }
            else {
                typeStr = 'undefined';
            }
    ...
    /**
     * 类型数组中是否包含某个具体的Type。注意，这里不是在做子集测试。
     * @param types
     * @param t
     */
    static contains(types, t) {
        for (let t1 of types) {
...
class ArrayType extends Type {
    ...
    toString() {
        let str = "" + this.baseType.toString() + "[]";
        return this.isComplement ? "!(" + str + ")" : str;
    }
    hasVoid() {
        return false;
    }
