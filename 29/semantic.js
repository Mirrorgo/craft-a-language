...
class Enter extends SemanticAstVisitor {
    ...
    visitClassDecl(classDecl) {
        ...
        //创建新的类型
        let theType = new types_1.NamedType(classDecl.name, []);
        //把类加入当前的符号表
        let sym = new symbol_1.ClassSymbol(classDecl, theType);
        classDecl.sym = sym;
        currentScope.enter(classDecl.name, sym);
        //在顶层Scope保存这个类型
        let prog = this.getRootNode(classDecl);
        (_a = prog.scope) === null || _a === void 0 ? void 0 : _a.type2sym.set(theType, sym);
        //继续遍历
    ...
    visitDotExp(dotExp) {
        ...
        if (dotExp.baseExp.theType) {
            ...
            if (t instanceof types_1.NamedType) {
                if (dotExp.property instanceof ast_1.Variable || dotExp.property instanceof ast_1.FunctionCall) {
                    let topScope = this.getRootNode(dotExp).scope;
                    let classSym = topScope.type2sym.get(t);
                    let classScope = classSym === null || classSym === void 0 ? void 0 : classSym.decl.body.scope;
                    if (classScope) {
