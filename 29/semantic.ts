...
class Enter extends SemanticAstVisitor{
    ...
    visitClassDecl(classDecl:ClassDecl){
        ...
        //创建新的类型
        let theType = new NamedType(classDecl.name,[]);  
        
        //把类加入当前的符号表
        let sym = new ClassSymbol(classDecl, theType);
        classDecl.sym = sym;
        currentScope.enter(classDecl.name, sym);

        //在顶层Scope保存这个类型
        let prog = this.getRootNode(classDecl);
        prog.scope?.type2sym.set(theType, sym);
...
class RefResolver extends SemanticAstVisitor{ 
    ...
    visitFunctionCall(functionCall:FunctionCall):any{
        ...
        if (built_ins.has(functionCall.name)){  //系统内置函数
            ...
            sym = currentScope.getSymbolCascade(functionCall.name) as FunctionSymbol|null;
        }
        ...
        if (!sym){
            ...
            if (sym instanceof ClassSymbol){
                ...
                let classScope = sym.decl.body.scope;

                functionCall.theType = sym.theType;   //类型是class

                if(classScope){
                    sym = classScope.getSymbol("constructor");
                }
                if (!sym){
    ...
    visitDotExp(dotExp: DotExp):any{
        ...
        if (dotExp.baseExp.theType){
            ...
            if (t instanceof NamedType){
                if(dotExp.property instanceof Variable || dotExp.property instanceof FunctionCall){
                    let topScope = this.getRootNode(dotExp).scope as Scope;
                    let classSym = topScope.type2sym.get(t);
                    let classScope = classSym?.decl.body.scope;
                    if (classScope){
                        let sym = classScope.getSymbol(dotExp.property.name);
                        if (sym){
                            ...
                            this.addError("Can not find property or method '" + dotExp.property.name + "' for class '" +t.name +"'", dotExp.property);
                        }
                    }
                }
                else{
        ...
        if (functionCall.sym != null){
            ...
            if(functionCall.sym.functionKind == FunctionKind.Constructor){
                ...
                if (classSym){
                    ...
                    this.addError("Can not find class symbol while creating an object of class : '"+functionCall.name+"'",functionCall);
                }
            }
            else{
