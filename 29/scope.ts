...
import {Symbol, SymKind, SymbolDumper, ClassSymbol} from './symbol';
import {AstNode, AstVisitor, Block, FunctionDecl, ForStatement, ClassDecl} from './ast'
...
export class Scope{
    ...
    //在本级Scope中新声明的类型，应该只在顶级的scope中保存
    type2sym:Map<Type,ClassSymbol> = new Map();

    constructor(node:AstNode, enclosingScope: Scope|null){
        this.node = node;
    ...
    getSymbolCascade(name:string):Symbol|null{
        ...
        if (sym != null){
            ...
            let node = this.enclosingScope.node;
            console.log("getSymbolCascade");
            console.log(node.toString);
            if (node.parentNode instanceof ClassDecl){  //类的属性不能被方法直接访问，必须使用this关键字
                return null;
            }
            else{
