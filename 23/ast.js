...
class AstDumper extends AstVisitor {
    ...
    visitBinary(exp, prefix) {
        console.log(prefix + "Binary:" + scanner_1.Op[exp.op] + (exp.theType == null ? "" : "(" + exp.theType.toString() + ")") + (exp.isErrorNode ? " **E** " : ""));
        this.visit(exp.exp1, prefix + "    ");
        this.visit(exp.exp2, prefix + "    ");
    }
    visitUnary(exp, prefix) {
        console.log(prefix + (exp.isPrefix ? "Prefix " : "PostFix ") + "Unary:" + scanner_1.Op[exp.op] + (exp.theType == null ? "" : "(" + exp.theType.toString() + ")") + (exp.isErrorNode ? " **E** " : ""));
    ...
    visitVariable(variable, prefix) {
        console.log(prefix + "Variable: " + (variable.isErrorNode ? " **E** " : "") + variable.name + (variable.theType == null ? "" : "(" + variable.theType.toString() + ")") + (variable.isLeftValue ? ", LeftValue" : "") + (variable.sym != null ? ", resolved" : ", not resolved"));
    }
