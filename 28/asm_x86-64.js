...
OpCodeUtil.muls = [OpCode.mull, OpCode.mulq, OpCode.mulsd];
OpCodeUtil.divs = [OpCode.divl, OpCode.divq, OpCode.divsd];
...
class AsmGenerator extends ast_1.AstVisitor {
    ...
    visitArrayLiteral(literal) {
        ...
        if (arrOprand.kind == OprandKind.varIndex) {
            ...
            for (let i = 0; i < literal.exps.length; i++) {
                ...
                let dataType = getCpuDataType(exp.theType);
    ...
    visitFunctionCall(functionCall) {
        ...
        //对于尾递归和尾调用，不需要处理返回值，也不需要做变量的溢出和重新装载
            ...
            return Register.returnReg(getCpuDataType(functionCall.theType));
        }
    }
