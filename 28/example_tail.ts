 ...
 * 一个尾调用的测试程序。
 * 
 * 使用：make example_tail，生成asm文件和可执行文件。
 * 
 */
 ...
 */
function foo3(p1:number, p2:number, p3:number, p4:number, p5:number, p6:number, p7:number, p8:number):number{
    let x1:number = p1*p2;
    let x2:number = p3*p4;
    let x3:number = x1 + x2 + p5*p6 + bar(p7, p8);
    if(x2 > x3){
