...
export abstract class StmtWithScope extends Statement{
 ...
 * 函数声明节点
 */
export class FunctionDecl extends StmtWithScope implements Decl{ 
    name:string;
    ...
    constructor(beginPos:Position, name:string, callSignature:CallSignature, body:Block, functionKind:FunctionKind, isErrorNode:boolean = false){
        super(beginPos, body.endPos, isErrorNode);
        this.name = name;
 ...
 * 函数体
 */
export class Block extends StmtWithScope{
    stmts: Statement[];
 ...
 * 变量声明节点
 */
export class VariableDecl extends AstNode implements Decl{
    name:string;
    // letToken:Token;
 ...
 * For语句
 */
export class ForStatement extends StmtWithScope{
 ...
 */
export class ClassDecl extends AstNode implements Decl{
    classToken:Token;
    body:Block;
    ...
    constructor(classToken:Token, endPos:Position,name:string, body:Block, superClass:string|null = null, isErrorNode:boolean = false){
        super(classToken.pos, endPos, isErrorNode);
        this.name = name;
...
export abstract class AstVisitor{
    ...
    //获取该节点所属的Scope
    getEnclosingScope(node:AstNode):Scope{
        let parent = node.parentNode;
        while (parent && !(parent instanceof StmtWithScope)){
            parent = parent.parentNode;
        }
        assert(parent, "getEnclosingScope()中，parent不应该是null");
        return parent?.scope as Scope;
    }
