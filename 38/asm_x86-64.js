...
class AsmModule {
    constructor() {
        ...
        this.fun2StackLayout = new Map();
        //函数中每个变量的类型  todo 这里没有使用窄化后的类型
        this.fun2VarTyps = new Map();
    }
    /**
     * 输出代表该模块的asm文件的字符串。
     */
    ...
    writeDoubleLiterals(fun, funIndex) {
    ...
    //34节
    //把函数的栈桢布局信息输出到一个数据段
    writeFunctionMeta(fun) {
        let stackLayout = this.fun2StackLayout.get(fun);
        let str = "";
        let varTypes = this.fun2VarTyps.get(fun);
        ...
        for (let i = 0; i <= maxIndex; i++) {
            if (stackLayout.has(i)) {
                if (varTypes.has(i)) {
                    let memAddress = stackLayout.get(i);
                    let typeTag = this.getTypeFlag(varTypes.get(i));
    ...
    //根据符号的类型，生成代表类型的数字标记
    getTypeFlag(t) {
        if (t === types_1.SysTypes.Any || t === types_1.SysTypes.Void || t === types_1.SysTypes.Never) {
            console.log("unsupported type in getTypeFlag()");
            return 0;
        }
        else if (t === types_1.SysTypes.Number) {
            return 1;
        }
        else if (t === types_1.SysTypes.Integer) {
            return 2;
        }
        else if (t === types_1.SysTypes.Decimal) {
            return 3;
        }
        else if (t === types_1.SysTypes.Boolean) {
            return 4;
        }
        else if (t === types_1.SysTypes.String) {
            return 5;
        }
        else if (t instanceof types_1.ArrayType) {
            return 6;
        }
        else if (t instanceof types_1.FunctionType) {
            return 7;
        }
        else if (t instanceof types_1.NamedType) {
            return 8;
        }
        else {
            console.log("unsupported type in getTypeFlag()");
    ...
    //typeTag: 占8位 1个字节
    //offset：占24位 3个字节
    compactToHexString(varIndex, typeTag, offset) {
...
class AsmGenerator extends ast_1.AstVisitor {
    ...
    handleFunction(functionSym, block) {
        ...
        //保存第1个变量的类型
        if (functionSym.classSym)
            this.s.varTypes.set(0, functionSym.classSym.theType);
        //计算当前函数是不是叶子函数
        ...
        (_e = this.asmModule) === null || _e === void 0 ? void 0 : _e.fun2VarTyps.set(this.s.functionSym, this.s.varTypes);
        //恢复原来的状态信息
        this.s = s;
    }
    /**
    ...
    visitBinary(bi) {
        ...
        if (bi.op == scanner_1.Op.Plus || bi.op == scanner_1.Op.Minus || bi.op == scanner_1.Op.Multiply || bi.op == scanner_1.Op.Divide) {
            if (!(dest instanceof VarOprand)) {
                ...
                this.s.varTypes.set(dest.varIndex, bi.exp1.theType);
        ...
        switch (bi.op) {
            ...
            case scanner_1.Op.Assign: //'='
                ...
                if (dest instanceof VarOprand) {
                    ...
                    this.s.varTypes.set(tempVar.varIndex, bi.exp2.theType);
    ...
    visitUnary(u) {
        ...
        if (u.op == scanner_1.Op.Inc || u.op == scanner_1.Op.Dec) {
            ...
            if (u.isPrefix) { //前缀运算符
                ...
                this.s.varTypes.set(tempVar.varIndex, u.theType);
    ...
    visitVariable(variable) {
        if (this.s.functionSym != null && variable.sym != null) {
            if (variable.sym instanceof symbol_1.VarSymbol) {
                ...
                this.s.varTypes.set(varIndex, variable.theType);
    ...
    visitStringLiteral(stringLiteral) {
        ...
        if (this.asmModule != null) {
            ...
            this.s.varTypes.set(tempVar.varIndex, stringLiteral.theType);
        ...
        if (arrOprand.kind == OprandKind.varIndex) {
            ...
            for (let i = 0; i < literal.exps.length; i++) {
                ...
                if (src.kind == OprandKind.doubleIndex) {
                    ...
                    this.s.varTypes.set(tempVar.varIndex, types_1.SysTypes.Number);
    ...
    visitIndexedExp(exp) {
        ...
        if (obj instanceof VarOprand) {
            ...
            let tempVar = this.allocateTempVar(CpuDataType.double);
            this.s.varTypes.set(tempVar.varIndex, obj.baseExp.theType);
    ...
    getFuncitonCallResult(functionType) {
        ...
        if (functionType.returnType != types_1.SysTypes.Void) { //函数有返回值时
            ...
            this.s.varTypes.set(dest.varIndex, functionType.returnType);
...
class Lower {
    ...
    lowerFunction(functionSym, bbs, funIndex) {
        ...
        //保存变量在栈桢中的布局信息
        (_a = this.asmModule) === null || _a === void 0 ? void 0 : _a.fun2StackLayout.set(functionSym, this.spilledVars2Address);
        return newBBs;
    }
    lowerParams() {
        var _a;
    ...
    addPrologue(insts) {
        ...
        let functionMetaLabel = "_" + ((_a = this.functionSym) === null || _a === void 0 ? void 0 : _a.fullName) + ".meta@GOTPCREL(%rip)";
